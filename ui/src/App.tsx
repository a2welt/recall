...
function CapturePage({ onCreated, projects, defaultProject }: { onCreated: (idea: Idea) => void; projects: Project[]; defaultProject: string }) {
  ...
  const [decision, setDecision] = useState(""); const [why, setWhy] = useState(""); const [alternatives, setAlternatives] = useState(""); const [tradeoffs, setTradeoffs] = useState(""); const [evidence, setEvidence] = useState(""); const [outcome, setOutcome] = useState("");
  const submit = async () => {
    if (!content.trim() || saving) return;
    setSaving(true);
    try {
      const resolvedTopic = topic.trim() || inferMemoryTopic({ content });
      const decisionPayload = { decision: decision.trim(), why: why.trim(), alternatives: alternatives.trim(), tradeoffs: tradeoffs.trim(), evidence: evidence.trim(), outcome: outcome.trim() };
      const idea = await api<Idea>("/api/ideas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content, priority, category, topic: resolvedTopic, project_id: projectId || null, workflow_status: workflowStatus, ...decisionPayload }) });
      onCreated({ ...idea, priority, category, topic: resolvedTopic, project_id: projectId || null, workflow_status: workflowStatus, ...decisionPayload, created_at: new Date().toISOString(), source: "ui" });
  ...
  return <div className="content-page narrow"><div className="capture-panel"><span className="eyebrow"><PlusCircle size={16} /> New memory</span><h2>What should your future self remember?</h2><p>Capture a decision, idea, issue, or note. Recall automatically links it to your current repository and branch.</p><textarea autoFocus value={content} onChange={(event) => setContent(event.target.value)} placeholder="e.g. We chose event sourcing for payments because..." />
    <div className="decision-fields"><div className="field-group"><label>Decision <small>Optional, but best for code decisions</small></label><input value={decision} onChange={(event) => setDecision(event.target.value)} placeholder="e.g. Use PostgreSQL for reporting" /></div><div className="field-group"><label>Why <small>The reasoning future you needs</small></label><textarea value={why} onChange={(event) => setWhy(event.target.value)} placeholder="e.g. JSONB indexes make the reporting filters fast without a second datastore." /></div><div className="capture-context-row"><div className="field-group"><label>Rejected alternatives</label><textarea value={alternatives} onChange={(event) => setAlternatives(event.target.value)} placeholder="e.g. MongoDB rejected because joins/reporting would become harder." /></div><div className="field-group"><label>Tradeoffs</label><textarea value={tradeoffs} onChange={(event) => setTradeoffs(event.target.value)} placeholder="e.g. More migration discipline, but simpler operations." /></div></div><div className="capture-context-row"><div className="field-group"><label>Evidence</label><input value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="PR, issue, error, metric, or observation" /></div><div className="field-group"><label>Outcome</label><input value={outcome} onChange={(event) => setOutcome(event.target.value)} placeholder="Expected or observed result" /></div></div></div>
...
function DecisionFacts({ idea }: { idea: Idea }) { const facts = [["Decision", idea.decision], ["Why", idea.why], ["Alternatives", idea.alternatives], ["Tradeoffs", idea.tradeoffs], ["Evidence", idea.evidence], ["Outcome", idea.outcome]].filter(([, value]) => value?.trim()); if (!facts.length) return null; return <section className="decision-facts">{facts.map(([label, value]) => <div key={label}><strong>{label}</strong><p>{value}</p></div>)}</section>; }
...
function DetailDrawer({ idea, projects, generate, close, changed, deleted, resolved }: { idea: Idea; projects: Project[]; generate: () => void; close: () => void; changed: (patch: Partial<Idea>) => void; deleted: () => void; resolved: () => void }) {
  ...
  return <><button className="drawer-scrim" onClick={close} aria-label="Close detail" /><aside className="detail-drawer"><div className="drawer-head"><span className="eyebrow"><meta.icon size={16} /> {meta.label}</span><button onClick={close}><X /></button></div><div className="drawer-body"><div className="drawer-status"><span className={`status ${idea.status}`}>{idea.status}</span><span className={`priority ${getPriority(idea)}`}>{getPriority(idea)} priority</span></div><h2>{idea.content}</h2><button className="generate-handoff" onClick={generate}><Sparkles size={17} /><span><strong>Generate implementation package</strong><small>Create a spec, prompt, or skill for an AI coding tool</small></span><ArrowRight size={16} /></button><DecisionFacts idea={idea} />{idea.reason && <p className="reason"><Sparkles size={16} />{idea.reason}</p>}<div className="drawer-organize"><label>Move to project<select value={idea.project_id || ""} onChange={(event) => void assign(event.target.value)}><option value="">Inbox</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><label>Lifecycle<select value={idea.workflow_status || "backlog"} onChange={(event) => void move(event.target.value as WorkflowStatus)}>{(Object.keys(workflowMeta) as WorkflowStatus[]).map((status) => <option value={status} key={status}>{workflowMeta[status].label}</option>)}</select></label>{idea.project_id && <button className="remove-project" onClick={() => void remove()}><FolderX size={15} />Remove from project</button>}</div><dl><div><dt>Topic hub</dt><dd>{idea.topic || inferMemoryTopic(idea)}</dd></div><div><dt>Created</dt><dd>{shortDate(idea.created_at)}</dd></div><div><dt>Source</dt><dd>{idea.source || "CLI"}</dd></div><div><dt>Repository</dt><dd title={idea.context?.repo_path || ""}>{idea.context?.repo_path?.split(/[\\/]/).pop() || "?"}</dd></div><div><dt>Branch</dt><dd>{idea.context?.branch || "—"}</dd></div><div><dt>File</dt><dd>{idea.context?.file_path?.split(/[\\/]/).pop() || "—"}</dd></div></dl><div className="danger-zone">{confirmingDelete ? <div><span><strong>Delete this memory?</strong><small>This cannot be undone.</small></span><button onClick={() => setConfirmingDelete(false)}>Cancel</button><button className="confirm-delete" disabled={deleting} onClick={() => void removePermanently()}>{deleting ? "Deleting…" : "Delete"}</button></div> : <button onClick={() => setConfirmingDelete(true)}><Trash2 size={16} />Delete memory</button>}</div></div><div className="drawer-foot">{idea.status === "open" && <button className="primary-button" onClick={() => void resolve()}><Check size={17} />Mark resolved</button>}<button className="secondary-button" onClick={close}>Close</button></div></aside></>;
}
